...
- Эксперименты диссертационных модулей выполняются по четырём этапам и
  двухрежимному контракту из
  [`PSES-DISS-001`](../docs/dissertation-skill-evaluation-standard.md).
- Gold-разметка и журнал контролируемых мутаций не передаются генератору или
  критику. После раскрытия независимая работа становится регрессионной.
- До критики фиксируется качество текстового слоя. Повреждённый слой даёт
  `not_assessed`, а OCR считается новым входным представлением с новым хешем.
- Первый независимый прогон критика хранится в
  [`EXP-0028`](EXP-0028-independent-dissertation-critic/report.md).
- `report`, `docx-comments`, `docx-track-changes` и `docx-hybrid` являются
  разными выходными профилями; смена профиля создаёт новый эксперимент.
