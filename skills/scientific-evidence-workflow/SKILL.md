...
Use complete scientific sentences throughout every notebook and companion Markdown report; exclude telegraphic result fragments and implication arrows. Before each material figure, motivate the relation or assessment that must be examined without opening with a deictic announcement such as “Ниже представлен рисунок”. Place a standalone caption in the form `Рисунок N. <предметное название>` immediately after the figure. Follow it with unlabelled connected prose that refers to the numbered figure, separates observation from interpretation, and records the bounded inference and limitations.
...
Close each nonterminal stage with a standalone concluding paragraph separated from the result analysis. Let its first sentence perform the synthesis itself by naming the established dependence, comparison, calculation, or result rather than citing the figure that displayed it; do not require an introductory connector such as “Таким образом”. In that paragraph state the established result, remaining limitation or question, reason for the next operation, and either an explicit decision criterion or a forecast attributed to a named model, hypothesis, equation, protocol, or comparison. Never use an unattributed impersonal forecast such as “Ожидается ...” or “Ожидаются ...”. Audit repeated sentence and paragraph openings across the report: rewrite frequent opening n-grams through subject-specific sentence structure rather than rotating a small list of synonyms. The following stage must resolve the declared task.
