...
FIGURE_INTRO_RE = re.compile(
    ...
    r"[^.\n]{0,220}\b(?:позвол\w*|необходим\w*|использу\w*|требу\w*|служ\w*)\b|"
    r"(?:оценк\w*|проверк\w*|сопоставлен\w*|анализ\w*|выявлен\w*|определен\w*|робастност\w*|влияни\w*|согласованност\w*|чувствительност\w*)"
    r"[^.\n]{0,220}\b(?:оценива\w*|проверя\w*|сопоставля\w*|рассматрива\w*|требу\w*|устанавлива\w*|определя\w*|характеризу\w*)\b|"
    r"[А-ЯЁA-Z][^.\n]{0,260}\b(?:оценива\w*|проверя\w*|определя\w*|сопоставля\w*|"
    r"исследу\w*|выявля\w*|основыва\w*|характеризу\w*)\b)"
)
...
FIGURE_ANALYSIS_RE = re.compile(
    ...
    r"зависимост\w*|различи\w*|различа\w*|свидетельству\w*|подтвержда\w*|ограничен\w*|"
    r"интерпретир\w*|характеризу\w*|ранжир\w*|указыва\w*|пригодн\w*|"
    r"нельзя|требу\w*|провер\w*)"
)
...
BRIDGE_EXPECTATION_RE = re.compile(
    r"(?i)\b(?:позволит\s+(?:установить|определить|проверить)|прогнозир\w*|предсказыва\w*|предполага\w*|"
    r"критери\w*|признак\w*|аргумент\w*|подтвержд\w*|при\s+подтверждении|если|долж\w*|считается|принимается|означает|недопустим\w*|блокиру\w*|запрет\w*)\b"
)
